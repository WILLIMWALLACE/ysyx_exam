...
INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1>>(BITS(((u_int64_t) src2),5,0)));

//B type,conditional jump
INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = (src1==src2)?(s->pc+dest):(s->snpc));
INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = (src1!=src2)?(s->pc+dest):(s->snpc));
INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (src1>=src2)?(s->pc+dest):(s->snpc));
//basic logic,similar to B_type's condition
INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^ src2  );
INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = src1 | src2  );
INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = src1 & src2  );
INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2  );
INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (src1<((u_int64_t) src2))?1:0);
INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = (((u_int64_t)src1)<((u_int64_t) src2))?1:0);

//jump instruction
INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc+src1;              R(dest)=s->snpc);//yi jing +4)
INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src2+src1)&0xfffffffe;  R(dest)=(dest==0)?0:s->snpc);

//memory instruction & quit
INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = SEXT(Mr(src1 + src2, 1),8));

...
INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(dest) = BITS(BITS(src1,31,0)/BITS(src2,31,0),31,0));
INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(dest) = BITS(BITS(src1,31,0)%BITS(src2,31,0),31,0));
//shifter(r\l)
...
INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (src1<src2)?(s->pc+dest):(s->snpc));
